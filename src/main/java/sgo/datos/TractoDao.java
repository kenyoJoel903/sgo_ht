...
@Repository
public class TractoDao {
	private JdbcTemplate jdbcTemplate;
	private NamedParameterJdbcTemplate namedJdbcTemplate;
	public static final String NOMBRE_TABLA = Constante.ESQUEMA_APLICACION + "tracto";
	public static final String NOMBRE_VISTA = Constante.ESQUEMA_APLICACION + "v_tracto";
	public final static String NOMBRE_CAMPO_CLAVE = "id_tracto";
	...
	public String mapearCampoOrdenamiento(String propiedad){
		...
		try {
			...
			if (propiedad.equals("razonSocialTransportista")){
				campoOrdenamiento="razon_social_transportista";
			}
	...
	public RespuestaCompuesta recuperarRegistros(ParametrosListar argumentosListar) {
		...
		Contenido<Tracto> contenido = new Contenido<Tracto>();
		List<Tracto> listaRegistros = new ArrayList<Tracto>();
		...
		try {
			...
			listaRegistros = jdbcTemplate.query(consultaSQL.toString(),parametros.toArray(), new TractoMapper());
	...
	public RespuestaCompuesta recuperarRegistro(int ID){
		...
		try {
			...
			consultaSQL.append("=?");
			listaRegistros= jdbcTemplate.query(consultaSQL.toString(),new Object[] {ID},new TractoMapper());
	...
	public RespuestaCompuesta validaRegistro(String placaTracto) {
		StringBuilder consultaSQL= new StringBuilder();		
		List<Tracto> listaRegistros=new ArrayList<Tracto>();
		Contenido<Tracto> contenido = new Contenido<Tracto>();
		RespuestaCompuesta respuesta= new RespuestaCompuesta();
		try {
			...
			consultaSQL.append(" WHERE ");
			consultaSQL.append(" t1.placa = ?");
			listaRegistros= jdbcTemplate.query(consultaSQL.toString(),new Object[] {placaTracto},new TractoMapper());
			contenido.totalRegistros=listaRegistros.size();
			contenido.totalEncontrados=listaRegistros.size();
			contenido.carga= listaRegistros;
			respuesta.mensaje="OK";
	...
	public RespuestaCompuesta guardarRegistro(Tracto tracto){
		RespuestaCompuesta respuesta = new RespuestaCompuesta();
		StringBuilder consultaSQL= new StringBuilder();
		...
		try {
			...
			consultaSQL.append(" (placa,estado,id_transportista,codigo_referencia,creado_el,creado_por,actualizado_por,actualizado_el,ip_creacion,ip_actualizacion) ");
			consultaSQL.append(" VALUES (:Placa,:Estado,:IdTransportista,:CodigoReferencia,:CreadoEl,:CreadoPor,:ActualizadoPor,:ActualizadoEl,:IpCreacion,:IpActualizacion) ");
			MapSqlParameterSource listaParametros= new MapSqlParameterSource();   
			listaParametros.addValue("Placa", tracto.getPlaca());
			listaParametros.addValue("Estado", tracto.getEstado());
			listaParametros.addValue("IdTransportista", tracto.getIdTransportista());
			//listaParametros.addValue("SincronizadoEl", tracto.getSincronizadoEl());
			//listaParametros.addValue("FechaReferencia", tracto.getFechaReferencia());
			listaParametros.addValue("CodigoReferencia", tracto.getCodigoReferencia());
			listaParametros.addValue("CreadoEl", tracto.getCreadoEl());
			listaParametros.addValue("CreadoPor", tracto.getCreadoPor());
			listaParametros.addValue("ActualizadoPor", tracto.getActualizadoPor());
			listaParametros.addValue("ActualizadoEl", tracto.getActualizadoEl());
			listaParametros.addValue("IpCreacion", tracto.getIpCreacion());
			listaParametros.addValue("IpActualizacion", tracto.getIpActualizacion());
	...
	public RespuestaCompuesta actualizarRegistro(Tracto tracto){
		RespuestaCompuesta respuesta = new RespuestaCompuesta();
		StringBuilder consultaSQL= new StringBuilder();
		int cantidadFilasAfectadas=0;
		try {
			...
			listaParametros.addValue("ActualizadoPor", tracto.getActualizadoPor());
			listaParametros.addValue("IpActualizacion", tracto.getIpActualizacion());
			listaParametros.addValue("Id", tracto.getId());
			SqlParameterSource namedParameters= listaParametros;
			/*Ejecuta la consulta y retorna las filas afectadas*/
	...
	public RespuestaCompuesta ActualizarEstadoRegistro(Tracto tracto){
		RespuestaCompuesta respuesta = new RespuestaCompuesta();
		StringBuilder consultaSQL= new StringBuilder();
		int cantidadFilasAfectadas=0;
		try {
