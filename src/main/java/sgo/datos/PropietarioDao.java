...
public class PropietarioDao {
	...
	public static final String NOMBRE_TABLA = Constante.ESQUEMA_APLICACION + "propietario";
	public static final String NOMBRE_VISTA = Constante.ESQUEMA_APLICACION + "v_propietario";
	public final static String NOMBRE_CAMPO_CLAVE = "id_propietario";
	...
	public RespuestaCompuesta recuperarRegistros(ParametrosListar argumentosListar) {
		...
		Contenido<Propietario> contenido = new Contenido<Propietario>();
		List<Propietario> listaRegistros = new ArrayList<Propietario>();
		...
		try {
			...
			if (!argumentosListar.getPropietarioNombreCorto().isEmpty()){
				filtrosWhere.add("lower(t1.nombre_corto) like lower('%"+ argumentosListar.getPropietarioNombreCorto() +"%') ");
			}
			...
			listaRegistros = jdbcTemplate.query(consultaSQL.toString(),parametros.toArray(), new PropietarioMapper());
	...
	public RespuestaCompuesta recuperarRegistro(int ID){
			...
			try {
				...
				consultaSQL.append("=?");
				listaRegistros= jdbcTemplate.query(consultaSQL.toString(),new Object[] {ID},new PropietarioMapper());
	...
	public RespuestaCompuesta guardarRegistro(Propietario propietario){
		RespuestaCompuesta respuesta = new RespuestaCompuesta();
		StringBuilder consultaSQL= new StringBuilder();
		...
		try {
			...
			consultaSQL.append(" (razon_social,nombre_corto,ruc,tipo,estado,creado_por,creado_el,actualizado_por,actualizado_el,ip_creacion,ip_actualizacion) ");
			consultaSQL.append(" VALUES (:RazonSocial,:NombreCorto,:Ruc,:Tipo,:Estado,:CreadoPor,:CreadoEl,:ActualizadoPor,:ActualizadoEl,:IpCreacion,:IpActualizacion) ");
			MapSqlParameterSource listaParametros= new MapSqlParameterSource();   
			listaParametros.addValue("RazonSocial", propietario.getRazonSocial());
			listaParametros.addValue("NombreCorto", propietario.getNombreCorto());
			listaParametros.addValue("Ruc", propietario.getRuc());
			listaParametros.addValue("Tipo", propietario.getTipo());
			listaParametros.addValue("Estado", propietario.getEstado());
			listaParametros.addValue("CreadoPor", propietario.getCreadoPor());
			listaParametros.addValue("CreadoEl", propietario.getCreadoEl());
			listaParametros.addValue("ActualizadoPor", propietario.getActualizadoPor());
			listaParametros.addValue("ActualizadoEl", propietario.getActualizadoEl());
			listaParametros.addValue("IpCreacion", propietario.getIpCreacion());
			listaParametros.addValue("IpActualizacion", propietario.getIpActualizacion());
	...
	public RespuestaCompuesta actualizarRegistro(Propietario propietario){
		RespuestaCompuesta respuesta = new RespuestaCompuesta();
		StringBuilder consultaSQL= new StringBuilder();
		int cantidadFilasAfectadas=0;
		try {
			...
			listaParametros.addValue("ActualizadoPor", propietario.getActualizadoPor());
			listaParametros.addValue("IpActualizacion", propietario.getIpActualizacion());
			listaParametros.addValue("Id", propietario.getId());
			SqlParameterSource namedParameters= listaParametros;
			/*Ejecuta la consulta y retorna las filas afectadas*/
	...
	public RespuestaCompuesta ActualizarEstadoRegistro(Propietario propietario){
		RespuestaCompuesta respuesta = new RespuestaCompuesta();
		StringBuilder consultaSQL= new StringBuilder();
		int cantidadFilasAfectadas=0;
		try {
