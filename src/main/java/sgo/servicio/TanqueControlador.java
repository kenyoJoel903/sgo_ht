...
@Controller
public class TanqueControlador {
	@Autowired
	...
	//urls generales
	private static final String URL_GESTION_COMPLETA="/admin/tanque";
	private static final String URL_GESTION_RELATIVA="/tanque";
	private static final String URL_GUARDAR_COMPLETA="/admin/tanque/crear";
	private static final String URL_GUARDAR_RELATIVA="/tanque/crear";
	private static final String URL_LISTAR_COMPLETA="/admin/tanque/listar";
	private static final String URL_LISTAR_RELATIVA="/tanque/listar";
	private static final String URL_ACTUALIZAR_COMPLETA="/admin/tanque/actualizar";
	private static final String URL_ACTUALIZAR_RELATIVA="/tanque/actualizar";
	private static final String URL_RECUPERAR_COMPLETA="/admin/tanque/recuperar";
	private static final String URL_RECUPERAR_RELATIVA="/tanque/recuperar";
	private static final String URL_ACTUALIZAR_ESTADO_COMPLETA="/admin/tanque/actualizarEstado";
	private static final String URL_ACTUALIZAR_ESTADO_RELATIVA="/tanque/actualizarEstado";
    ...
    mapaValores.put("TANQUE_TIPO_ADMINISTRADO", String.valueOf(Tanque.TIPO_ADMINISTRADO));
    mapaValores.put("TANQUE_TIPO_VIRTUAL", String.valueOf(Tanque.TIPO_VIRTUAL));
    mapaValores.put("TEXTO_TIPO_ADMINISTRADO", gestorDiccionario.getMessage("sgo.tanqueTipoAdministrado",null,locale));
    mapaValores.put("TEXTO_TIPO_VIRTUAL", gestorDiccionario.getMessage("sgo.tanqueTipoVirtual",null,locale));
	 }catch(Exception ex){
	...
	public ModelAndView mostrarFormulario( Locale locale){
		...
		ArrayList<?> listadoEstaciones = null;
      ...
      if (respuesta.estado==false){
        throw new Exception(gestorDiccionario.getMessage("sgo.noListadoEstaciones",null,locale));
      	...
      	listadoEstaciones = (ArrayList<?>) respuesta.contenido.carga;
      	mapaValores = recuperarMapaValores(locale);
		vista = new ModelAndView("plantilla");
		vista.addObject("vistaJSP","mantenimiento/tanque.jsp");
		vista.addObject("vistaJS","mantenimiento/tanque.js");
		vista.addObject("identidadUsuario",principal.getIdentidad());
		vista.addObject("menu",listaEnlaces);
		vista.addObject("listadoEstaciones",listadoEstaciones);
	...
	public @ResponseBody RespuestaCompuesta guardarRegistro(@RequestBody Tanque eTanque,HttpServletRequest peticionHttp,Locale locale){		 
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		...
		try {
		    ...
		    Respuesta validacion = Utilidades.validacionXSS(eTanque, gestorDiccionario, locale);
		    if (validacion.estado == false) {
		      throw new Exception(validacion.valor);
		    }
			//Inicia la transaccion
			this.transaccion = new DataSourceTransactionManager(dTanque.getDataSource());
        	...
        	eTanque.setActualizadoEl(Calendar.getInstance().getTime().getTime());
            eTanque.setActualizadoPor(principal.getID()); 
           	eTanque.setCreadoEl(Calendar.getInstance().getTime().getTime());
            eTanque.setCreadoPor(principal.getID());
            eTanque.setIpActualizacion(direccionIp);
            eTanque.setIpCreacion(direccionIp);
            eTanque.setEstado(Constante.ESTADO_ACTIVO);
            respuesta= dTanque.guardarRegistro(eTanque);
            ...
            ContenidoAuditoria =  mapper.writeValueAsString(eTanque);
            eBitacora.setUsuario(principal.getNombre());
            eBitacora.setAccion(URL_GUARDAR_COMPLETA);
            eBitacora.setTabla(TanqueDao.NOMBRE_TABLA);
            eBitacora.setIdentificador(ClaveGenerada);
            eBitacora.setContenido(ContenidoAuditoria);
            eBitacora.setRealizadoEl(eTanque.getCreadoEl());
            eBitacora.setRealizadoPor(eTanque.getCreadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }           
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.guardarExitoso",new Object[] {  eTanque.getFechaCreacion().substring(0, 9),eTanque.getFechaCreacion().substring(10),principal.getIdentidad() },locale);
	...
	public @ResponseBody RespuestaCompuesta actualizarRegistro(@RequestBody Tanque eTanque,HttpServletRequest peticionHttp,Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		TransactionDefinition definicionTransaccion = null;
		TransactionStatus estadoTransaccion = null;
		...
		try {
            ...
            eTanque.setActualizadoPor(principal.getID()); 
            eTanque.setIpActualizacion(direccionIp);
            respuesta= dTanque.actualizarRegistro(eTanque);
            if (respuesta.estado==false){          	
            ...
            eBitacora.setTabla(TanqueDao.NOMBRE_TABLA);
            eBitacora.setIdentificador(String.valueOf( eTanque.getId()));
            eBitacora.setContenido( mapper.writeValueAsString(eTanque));
            eBitacora.setRealizadoEl(eTanque.getActualizadoEl());
            eBitacora.setRealizadoPor(eTanque.getActualizadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }  
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.actualizarExitoso",new Object[] {  eTanque.getFechaActualizacion().substring(0, 9),eTanque.getFechaActualizacion().substring(10),principal.getIdentidad() },locale);;
        	this.transaccion.commit(estadoTransaccion);
		} catch (Exception ex){
			ex.printStackTrace();
			this.transaccion.rollback(estadoTransaccion);
	...
	public @ResponseBody RespuestaCompuesta actualizarEstadoRegistro(@RequestBody Tanque eTanque,HttpServletRequest peticionHttp,Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		TransactionDefinition definicionTransaccion = null;
		TransactionStatus estadoTransaccion = null;
		...
		try {
			...
			eTanque.setActualizadoPor(principal.getID()); 
			eTanque.setIpActualizacion(direccionIp);
            respuesta= dTanque.ActualizarEstadoRegistro(eTanque);
            if (respuesta.estado==false){          	
            	throw new Exception(gestorDiccionario.getMessage("sgo.actualizarFallido",null,locale));
            }
            //Guardar en la bitacora
