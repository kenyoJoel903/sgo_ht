...
public class ProductoControlador {
	...
	//urls generales
	private static final String URL_GESTION_COMPLETA="/admin/producto";
	private static final String URL_GESTION_RELATIVA="/producto";
	private static final String URL_GUARDAR_COMPLETA="/admin/producto/crear";
	private static final String URL_GUARDAR_RELATIVA="/producto/crear";
	private static final String URL_LISTAR_COMPLETA="/admin/producto/listar";
	private static final String URL_LISTAR_RELATIVA="/producto/listar";
	private static final String URL_ACTUALIZAR_COMPLETA="/admin/producto/actualizar";
	private static final String URL_ACTUALIZAR_RELATIVA="/producto/actualizar";
	private static final String URL_RECUPERAR_COMPLETA="/admin/producto/recuperar";
	private static final String URL_RECUPERAR_RELATIVA="/producto/recuperar";
	private static final String URL_ACTUALIZAR_ESTADO_COMPLETA="/admin/producto/actualizarEstado";
	private static final String URL_ACTUALIZAR_ESTADO_RELATIVA="/producto/actualizarEstado";
	private static final String URL_LISTAR_POR_OPERACION_COMPLETA="/admin/producto/listarPorOperacion";
	private static final String URL_LISTAR_POR_OPERACION_RELATIVA="/producto/listarPorOperacion";
	...
	public ModelAndView mostrarFormulario( Locale locale){
		...
		try {
			...
			vista.addObject("vistaJSP","mantenimiento/producto.jsp");
			vista.addObject("vistaJS","mantenimiento/producto.js");
	...
	@RequestMapping(value = URL_LISTAR_POR_OPERACION_RELATIVA ,method = RequestMethod.GET)
	public @ResponseBody RespuestaCompuesta recuperarRegistrosPorOperacion(HttpServletRequest httpRequest, Locale locale){
		RespuestaCompuesta respuesta = null;
		ParametrosListar parametros= null;
		AuthenticatedUserDetails principal = null;
		String mensajeRespuesta="";
		try {
			...
			principal = this.getCurrentUser(); 
			//Recuperar el enlace de la accion
			respuesta = dEnlace.recuperarRegistro(URL_LISTAR_POR_OPERACION_COMPLETA);
			if (respuesta.estado==false){
				mensajeRespuesta = gestorDiccionario.getMessage("sgo.accionNoHabilitada",null,locale);
				throw new Exception(mensajeRespuesta);
			}
			...
			parametros.setIndicadorProducto(Constante.FILTRO_TODOS);
			if (httpRequest.getParameter("indicadorProducto") != null) {
				parametros.setIndicadorProducto(Integer.parseInt(httpRequest.getParameter("indicadorProducto")));
			}
			//Recuperar registros
			parametros.setPaginacion(Constante.SIN_PAGINACION);
			respuesta = dProducto.recuperarRegistrosPorOperacion(parametros);
	
			//esto es para agregar el sin producto en el listado de productos
			if(parametros.getIndicadorProducto() == Producto.INDICADOR_PRODUCTO_SIN_DATOS){
				...
				parametros2.setIndicadorProducto(parametros.getIndicadorProducto());
				RespuestaCompuesta respuestaSinProducto = dProducto.recuperarRegistros(parametros2);
				if (respuesta.estado==false){        	
	            	throw new Exception(gestorDiccionario.getMessage("sgo.recuperarFallido",null,locale));
	            }
				for (int i = 0; i < respuesta.contenido.carga.size(); i++){
					Producto eProducto = (Producto) respuesta.contenido.carga.get(i);
					listaRegistros.add(eProducto);
				}
				//esto para agregar el sin producto al listado
				listaRegistros.add((Producto) respuestaSinProducto.contenido.carga.get(0));
	...
	public @ResponseBody RespuestaCompuesta recuperaRegistro(int ID,Locale locale){
		...
		try {			
        	...
        	respuesta= dProducto.recuperarRegistro(ID);
	...
	public @ResponseBody RespuestaCompuesta guardarRegistro(@RequestBody Producto eProducto,HttpServletRequest peticionHttp,Locale locale){		 
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		...
		try {
		    ...
		    Respuesta validacion = Utilidades.validacionXSS(eProducto, gestorDiccionario, locale);
		    if (validacion.estado == false) {
		      throw new Exception(validacion.valor);
		    }
			//Inicia la transaccion
			this.transaccion = new DataSourceTransactionManager(dProducto.getDataSource());
			...
			eProducto.setIndicadorProducto(Producto.INDICADOR_PRODUCTO_CON_DATOS);
        	eProducto.setActualizadoEl(Calendar.getInstance().getTime().getTime());
            eProducto.setActualizadoPor(principal.getID()); 
           	eProducto.setCreadoEl(Calendar.getInstance().getTime().getTime());
            eProducto.setCreadoPor(principal.getID());
            eProducto.setIpActualizacion(direccionIp);
            eProducto.setIpCreacion(direccionIp);
            eProducto.setEstado(Constante.ESTADO_ACTIVO);
            respuesta= dProducto.guardarRegistro(eProducto);
            ...
            ContenidoAuditoria =  mapper.writeValueAsString(eProducto);
            eBitacora.setUsuario(principal.getNombre());
            eBitacora.setAccion(URL_GUARDAR_COMPLETA);
            eBitacora.setTabla(ProductoDao.NOMBRE_TABLA);
            eBitacora.setIdentificador(ClaveGenerada);
            eBitacora.setContenido(ContenidoAuditoria);
            eBitacora.setRealizadoEl(eProducto.getCreadoEl());
            eBitacora.setRealizadoPor(eProducto.getCreadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }           
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.guardarExitoso",new Object[] {  eProducto.getFechaCreacion().substring(0, 9),eProducto.getFechaCreacion().substring(10),principal.getIdentidad() },locale);
	...
	public @ResponseBody RespuestaCompuesta actualizarRegistro(@RequestBody Producto eProducto,HttpServletRequest peticionHttp,Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		TransactionDefinition definicionTransaccion = null;
		TransactionStatus estadoTransaccion = null;
		...
		try {
            ...
            eProducto.setActualizadoPor(principal.getID()); 
            eProducto.setIpActualizacion(direccionIp);
            respuesta= dProducto.actualizarRegistro(eProducto);
            if (respuesta.estado==false){          	
            ...
            eBitacora.setTabla(ProductoDao.NOMBRE_TABLA);
            eBitacora.setIdentificador(String.valueOf( eProducto.getId()));
            eBitacora.setContenido( mapper.writeValueAsString(eProducto));
            eBitacora.setRealizadoEl(eProducto.getActualizadoEl());
            eBitacora.setRealizadoPor(eProducto.getActualizadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }  
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.actualizarExitoso",new Object[] {  eProducto.getFechaActualizacion().substring(0, 9),eProducto.getFechaActualizacion().substring(10),principal.getIdentidad() },locale);;
        	this.transaccion.commit(estadoTransaccion);
		} catch (Exception ex){
			ex.printStackTrace();
			this.transaccion.rollback(estadoTransaccion);
	...
	public @ResponseBody RespuestaCompuesta actualizarEstadoRegistro(@RequestBody Producto eEntidad,HttpServletRequest peticionHttp,Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		TransactionDefinition definicionTransaccion = null;
		TransactionStatus estadoTransaccion = null;
		...
		try {
            ...
            respuesta= dProducto.ActualizarEstadoRegistro(eEntidad);
            if (respuesta.estado==false){          	
            	throw new Exception(gestorDiccionario.getMessage("sgo.actualizarFallido",null,locale));
            }
            //Guardar en la bitacora
