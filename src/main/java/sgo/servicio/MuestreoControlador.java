...
@Controller
public class MuestreoControlador {
	@Autowired
	...
	//urls generales
	private static final String URL_GESTION_COMPLETA="/admin/muestreo";
	private static final String URL_GESTION_RELATIVA="/muestreo";
	private static final String URL_GUARDAR_COMPLETA="/admin/muestreo/crear";
	private static final String URL_GUARDAR_RELATIVA="/muestreo/crear";
	private static final String URL_LISTAR_COMPLETA="/admin/muestreo/listar";
	private static final String URL_LISTAR_RELATIVA="/muestreo/listar";
	private static final String URL_ACTUALIZAR_COMPLETA="/admin/muestreo/actualizar";
	private static final String URL_ACTUALIZAR_RELATIVA="/muestreo/actualizar";
	private static final String URL_RECUPERAR_COMPLETA="/admin/muestreo/recuperar";
	private static final String URL_RECUPERAR_RELATIVA="/muestreo/recuperar";
	...
	public ModelAndView mostrarFormulario( Locale locale){
		...
		try {
			...
			vista.addObject("vistaJSP","mantenimiento/muestreo.jsp");
			vista.addObject("vistaJS","mantenimiento/muestreo.js");
	...
	public @ResponseBody RespuestaCompuesta recuperaRegistro(int ID,Locale locale){
		...
		try {			
        	...
        	respuesta= dMuestreo.recuperarRegistro(ID);
	...
	public @ResponseBody RespuestaCompuesta guardarRegistro(@RequestBody Muestreo eMuestreo,HttpServletRequest peticionHttp,Locale locale){		 
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		...
		try {
			...
			this.transaccion = new DataSourceTransactionManager(dMuestreo.getDataSource());
        	...
        	eMuestreo.setActualizadoEl(Calendar.getInstance().getTime().getTime());
            eMuestreo.setActualizadoPor(principal.getID()); 
           	eMuestreo.setCreadoEl(Calendar.getInstance().getTime().getTime());
            eMuestreo.setCreadoPor(principal.getID());
            eMuestreo.setIpActualizacion(direccionIp);
            eMuestreo.setIpCreacion(direccionIp);
            respuesta= dMuestreo.guardarRegistro(eMuestreo);
            ...
            ContenidoAuditoria =  mapper.writeValueAsString(eMuestreo);
            ...
            eBitacora.setRealizadoEl(eMuestreo.getCreadoEl());
            eBitacora.setRealizadoPor(eMuestreo.getCreadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }           
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.guardarExitoso",new Object[] {  eMuestreo.getFechaCreacion().substring(0, 9),eMuestreo.getFechaCreacion().substring(10),principal.getIdentidad() },locale);
	...
	public @ResponseBody RespuestaCompuesta actualizarRegistro(@RequestBody Muestreo eMuestreo,HttpServletRequest peticionHttp,Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		TransactionDefinition definicionTransaccion = null;
		TransactionStatus estadoTransaccion = null;
		...
		try {
            ...
            eMuestreo.setActualizadoPor(principal.getID()); 
            eMuestreo.setIpActualizacion(direccionIp);
            respuesta= dMuestreo.actualizarRegistro(eMuestreo);
            if (respuesta.estado==false){          	
            ...
            eBitacora.setTabla(MuestreoDao.NOMBRE_TABLA);
            eBitacora.setIdentificador(String.valueOf( eMuestreo.getId()));
            eBitacora.setContenido( mapper.writeValueAsString(eMuestreo));
            eBitacora.setRealizadoEl(eMuestreo.getActualizadoEl());
            eBitacora.setRealizadoPor(eMuestreo.getActualizadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }  
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.actualizarExitoso",new Object[] {  eMuestreo.getFechaActualizacion().substring(0, 9),eMuestreo.getFechaActualizacion().substring(10),principal.getIdentidad() },locale);;
        	this.transaccion.commit(estadoTransaccion);
		} catch (Exception ex){
			ex.printStackTrace();
			this.transaccion.rollback(estadoTransaccion);
