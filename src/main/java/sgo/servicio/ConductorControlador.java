...
public class ConductorControlador {
 ...
 // urls generales
 private static final String URL_GESTION_COMPLETA = "/admin/conductor";
 private static final String URL_GESTION_RELATIVA = "/conductor";
 private static final String URL_GUARDAR_COMPLETA = "/admin/conductor/crear";
 private static final String URL_GUARDAR_RELATIVA = "/conductor/crear";
 private static final String URL_LISTAR_COMPLETA = "/admin/conductor/listar";
 private static final String URL_LISTAR_RELATIVA = "/conductor/listar";
 private static final String URL_ACTUALIZAR_COMPLETA = "/admin/conductor/actualizar";
 private static final String URL_ACTUALIZAR_RELATIVA = "/conductor/actualizar";
 private static final String URL_RECUPERAR_COMPLETA = "/admin/conductor/recuperar";
 private static final String URL_RECUPERAR_RELATIVA = "/conductor/recuperar";
 private static final String URL_ACTUALIZAR_ESTADO_COMPLETA = "/admin/conductor/actualizarEstado";
 private static final String URL_ACTUALIZAR_ESTADO_RELATIVA = "/conductor/actualizarEstado";
 ...
 public ModelAndView mostrarFormulario(Locale locale) {
  ...
  try {
   ...
   vista.addObject("vistaJSP", "mantenimiento/conductor.jsp");
   vista.addObject("vistaJS", "mantenimiento/conductor.js");
 ...
 RespuestaCompuesta recuperarRegistros(HttpServletRequest httpRequest, Locale locale) {
  ...
  try {
   ...
   // Recuperar registros
   respuesta = dConductor.recuperarRegistros(parametros);
 ...
 RespuestaCompuesta recuperaRegistro(int ID, Locale locale) {
  ...
  try {
   ...
   respuesta = dConductor.recuperarRegistro(ID);
 ...
 public @ResponseBody
 RespuestaCompuesta guardarRegistro(@RequestBody Conductor eConductor, HttpServletRequest peticionHttp, Locale locale) {
  RespuestaCompuesta respuesta = null;
  AuthenticatedUserDetails principal = null;
  ...
  try {
    ...
    Respuesta validacion = Utilidades.validacionXSS(eConductor, gestorDiccionario, locale);
    if (validacion.estado == false) {
      throw new Exception(validacion.valor);
    }
   // Inicia la transaccion
   this.transaccion = new DataSourceTransactionManager(dConductor.getDataSource());
   ...
   eConductor.setActualizadoEl(Calendar.getInstance().getTime().getTime());
   eConductor.setActualizadoPor(principal.getID());
   eConductor.setCreadoEl(Calendar.getInstance().getTime().getTime());
   eConductor.setCreadoPor(principal.getID());
   eConductor.setIpActualizacion(direccionIp);
   eConductor.setIpCreacion(direccionIp);
   eConductor.setEstado(Constante.ESTADO_ACTIVO);
   respuesta = dConductor.guardarRegistro(eConductor);
   ...
   ObjectMapper mapper = new ObjectMapper(); // no need to do this if you inject
                                             // via @Autowired
   ContenidoAuditoria = mapper.writeValueAsString(eConductor);
   eBitacora.setUsuario(principal.getNombre());
   eBitacora.setAccion(URL_GUARDAR_COMPLETA);
   eBitacora.setTabla(ConductorDao.NOMBRE_TABLA);
   eBitacora.setIdentificador(ClaveGenerada);
   eBitacora.setContenido(ContenidoAuditoria);
   eBitacora.setRealizadoEl(eConductor.getCreadoEl());
   eBitacora.setRealizadoPor(eConductor.getCreadoPor());
   respuesta = dBitacora.guardarRegistro(eBitacora);
   if (respuesta.estado == false) {
    throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido", null, locale));
   }
   respuesta.mensaje = gestorDiccionario.getMessage("sgo.guardarExitoso",
     new Object[] { eConductor.getFechaCreacion().substring(0, 9), eConductor.getFechaCreacion().substring(10), principal.getIdentidad() }, locale);
 ...
 public @ResponseBody
 RespuestaCompuesta actualizarRegistro(@RequestBody Conductor eConductor, HttpServletRequest peticionHttp, Locale locale) {
  RespuestaCompuesta respuesta = null;
  AuthenticatedUserDetails principal = null;
  TransactionDefinition definicionTransaccion = null;
  TransactionStatus estadoTransaccion = null;
  ...
  try {
   ...
   eConductor.setActualizadoPor(principal.getID());
   eConductor.setIpActualizacion(direccionIp);
   respuesta = dConductor.actualizarRegistro(eConductor);
   if (respuesta.estado == false) {
   ...
   eBitacora.setTabla(ConductorDao.NOMBRE_TABLA);
   eBitacora.setIdentificador(String.valueOf(eConductor.getId()));
   eBitacora.setContenido(mapper.writeValueAsString(eConductor));
   eBitacora.setRealizadoEl(eConductor.getActualizadoEl());
   eBitacora.setRealizadoPor(eConductor.getActualizadoPor());
   respuesta = dBitacora.guardarRegistro(eBitacora);
   if (respuesta.estado == false) {
    throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido", null, locale));
   }
   respuesta.mensaje = gestorDiccionario.getMessage("sgo.actualizarExitoso", new Object[] { eConductor.getFechaActualizacion().substring(0, 9), eConductor.getFechaActualizacion().substring(10),
 ...
 public @ResponseBody
 RespuestaCompuesta actualizarEstadoRegistro(@RequestBody Conductor eConductor, HttpServletRequest peticionHttp, Locale locale) {
  RespuestaCompuesta respuesta = null;
  AuthenticatedUserDetails principal = null;
  TransactionDefinition definicionTransaccion = null;
  TransactionStatus estadoTransaccion = null;
  ...
  try {
   ...
   eConductor.setActualizadoPor(principal.getID());
   eConductor.setIpActualizacion(direccionIp);
   respuesta = dConductor.ActualizarEstadoRegistro(eConductor);
   if (respuesta.estado == false) {
    throw new Exception(gestorDiccionario.getMessage("sgo.actualizarFallido", null, locale));
   }
   // Guardar en la bitacora
