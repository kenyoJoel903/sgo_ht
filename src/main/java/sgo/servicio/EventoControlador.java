...
public class EventoControlador {
	...
	//urls generales
	private static final String URL_GESTION_COMPLETA="/admin/evento";
	private static final String URL_GESTION_RELATIVA="/evento";
	private static final String URL_GUARDAR_COMPLETA="/admin/evento/crear";
	private static final String URL_GUARDAR_RELATIVA="/evento/crear";
	private static final String URL_LISTAR_COMPLETA="/admin/evento/listar";
	private static final String URL_LISTAR_RELATIVA="/evento/listar";
	private static final String URL_ACTUALIZAR_COMPLETA="/admin/evento/actualizar";
	private static final String URL_ACTUALIZAR_RELATIVA="/evento/actualizar";
	private static final String URL_RECUPERAR_COMPLETA="/admin/evento/recuperar";
	private static final String URL_RECUPERAR_RELATIVA="/evento/recuperar";
	private static final String URL_RECUPERAR_X_REGISTRO_Y_TIPO_COMPLETA="/admin/evento/recuperarXregistroYtipo";
	private static final String URL_RECUPERAR_X_REGISTRO_Y_TIPO_RELATIVA="/evento/recuperarXregistroYtipo";
	...
	public ModelAndView mostrarFormulario( Locale locale){
		...
		try {
			...
			vista.addObject("vistaJSP","mantenimiento/evento.jsp");
			vista.addObject("vistaJS","mantenimiento/evento.js");
	...
	public @ResponseBody RespuestaCompuesta recuperarRegistros(HttpServletRequest httpRequest, Locale locale){
		...
		try {
			...
			//Recuperar registros
			respuesta = dEvento.recuperarRegistros(parametros);
	...
	public @ResponseBody RespuestaCompuesta recuperaRegistro(int ID,Locale locale){
		...
		try {			
        	...
        	respuesta= dEvento.recuperarRegistro(ID);
	...
	@RequestMapping(value = URL_RECUPERAR_X_REGISTRO_Y_TIPO_RELATIVA ,method = RequestMethod.GET)
	public @ResponseBody RespuestaCompuesta recuperaRegistro(int IdRegistro, int tipoRegistro, Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		try {			
			//Recupera el usuario actual
			principal = this.getCurrentUser(); 
			//Recuperar el enlace de la accion
			respuesta = dEnlace.recuperarRegistro(URL_RECUPERAR_X_REGISTRO_Y_TIPO_COMPLETA);
			if (respuesta.estado==false){
				throw new Exception(gestorDiccionario.getMessage("sgo.accionNoHabilitada",null,locale));
			}
			Enlace eEnlace = (Enlace) respuesta.getContenido().getCarga().get(0);
			...
			//Recuperar el registro
        	respuesta= dEvento.recuperarRegistroPorIdRegistroYTipoRegistro(IdRegistro, tipoRegistro); //se envía 1 porque el tipo de registro es transporte
        	//Verifica el resultado de la accion
            if (respuesta.estado==false){        	
            	throw new Exception(gestorDiccionario.getMessage("sgo.recuperarFallido",null,locale));
            }
	...
	public @ResponseBody RespuestaCompuesta guardarRegistro(@RequestBody Evento eEvento,HttpServletRequest peticionHttp,Locale locale){		 
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		...
		try {
			...
			this.transaccion = new DataSourceTransactionManager(dEvento.getDataSource());
        	...
        	eEvento.setActualizadoEl(Calendar.getInstance().getTime().getTime());
            eEvento.setActualizadoPor(principal.getID()); 
           	eEvento.setCreadoEl(Calendar.getInstance().getTime().getTime());
            eEvento.setCreadoPor(principal.getID());
            eEvento.setIpActualizacion(direccionIp);
	...
	public @ResponseBody RespuestaCompuesta actualizarRegistro(@RequestBody Evento eEvento,HttpServletRequest peticionHttp,Locale locale){
		RespuestaCompuesta respuesta = null;
		AuthenticatedUserDetails principal = null;
		TransactionDefinition definicionTransaccion = null;
		TransactionStatus estadoTransaccion = null;
		...
		try {
            ...
            eEvento.setActualizadoPor(principal.getID()); 
            eEvento.setIpActualizacion(direccionIp);
            respuesta= dEvento.actualizarRegistro(eEvento);
            if (respuesta.estado==false){          	
            ...
            eBitacora.setTabla(EventoDao.NOMBRE_TABLA);
            eBitacora.setIdentificador(String.valueOf( eEvento.getId()));
            eBitacora.setContenido( mapper.writeValueAsString(eEvento));
            eBitacora.setRealizadoEl(eEvento.getActualizadoEl());
            eBitacora.setRealizadoPor(eEvento.getActualizadoPor());
            respuesta= dBitacora.guardarRegistro(eBitacora);
            if (respuesta.estado==false){     	
              	throw new Exception(gestorDiccionario.getMessage("sgo.guardarBitacoraFallido",null,locale));
            }  
        	respuesta.mensaje=gestorDiccionario.getMessage("sgo.actualizarExitoso",new Object[] {  eEvento.getFechaActualizacion().substring(0, 9),eEvento.getFechaActualizacion().substring(10),principal.getIdentidad() },locale);;
        	this.transaccion.commit(estadoTransaccion);
		} catch (Exception ex){
			ex.printStackTrace();
			this.transaccion.rollback(estadoTransaccion);
